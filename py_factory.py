...
class DummyModule(nn.Module):
    ...
    def forward(self, *xs, **kwargs):
        return self.module(*xs, **kwargs)  # saccade_net.forward
...
class NetworkFactory(object):
    def __init__(self, system_config, model, distributed=False, gpu=None):
        ...
        self.loss    = model.loss   # CornerNet_Saccade_Loss()
        ...
        if distributed:
            ...
            #self.network = self.network  #hccho
    ...
    def _t_cuda(self, xs):
        if torch.cuda.is_available():
            ...
            else: return xs.cuda(self.gpu, non_blocking=True)
    ...
    def loss2(self, xs, ys, **kwargs):
        xs = [self._t_cuda(x) for x in xs]
        ys = [self._t_cuda(y) for y in ys]

        loss = self.network(xs, ys)  #foward
        loss = loss.mean()
    ...
    def load_pretrained_params(self, pretrained_model):
        ...
        with open(pretrained_model, "rb") as f:
            if torch.cuda.is_available():
                params = torch.load(f)
            else: params = torch.load(f,map_location='cpu')
            model_state = self.model.state_dict()
            pretrained_state = { k:v for k,v in params.items() if k in model_state and v.size() == model_state[k].size() }
